...
class ChartDataContainer : ObservableObject {
    ...
    func calculatePercentages(){
        ...
        for crypto in strongService.portfolio {
            group.enter()
            APICaller.getSingleDetailsCrypto(cryptoID: SupportedCrypto.getCryptoKeyForAPI(key: crypto.cryptoID ?? "")){ [weak self] (response) in
        ...
        group.notify(queue: .main) {
            for asset in strongService.portfolio {
                APICaller.getSingleDetailsCrypto(cryptoID: SupportedCrypto.getCryptoKeyForAPI(key: asset.cryptoID ?? "")){ [weak self] (response) in
                    guard let self = self else {
                        return
                    }
                    DispatchQueue.main.async {
