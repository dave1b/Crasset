...
class ChartDataContainer : ObservableObject {
    ...
    @Published var chartData : [ChartData] = [ChartData(color: Color(#colorLiteral(red: 1, green: 0.4932718873, blue: 0.4739984274, alpha: 1)), percent: 10, value: 10, coinID: "unknown")]
        ...
        group.notify(queue: .main) {
            for asset in strongService.portfolio {
                APICaller.getSingleDetailsCrypto(cryptoID: SupportedCrypto.getCryptoKeyForAPI(key: asset.coinID ?? "")){ [weak self] (response) in
                    ...
                    self.chartData.append(ChartData(color: self.colorArray[index%6], percent: (CGFloat(asset.amount * priceInUSD / self.totalInUSD) * 100), value: (CGFloat(asset.amount * priceInUSD / self.totalInUSD) * 100), coinID:  asset.coinID ?? "unknown"))
                        self.calc()
                        index = index+1
